...
export default function RecentPage() {
  ...
  useEffect(() => {
    ...
    apiClient.get<{ data: Entity[] }>(`/entities/?workspace_id=${workspaceId}&sort=updated_at&order=desc&limit=20`)
      .then((json) => {
  ...
  return (
    <div className="mx-auto max-w-4xl p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-500">Recently updated entities</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-12 text-center">
          ...
          <p className="mt-2 text-sm text-zinc-500">No recent entities</p>
        </div>
      ) : (
          ...
          {entities.map((entity) => (
            ...
            >
              <div>
                ...
                <p className="text-xs text-zinc-500">Updated {formatRelativeTime(entity.updated_at)}</p>
              </div>
            </Link>
          ))}
        </div>
