...
const CONFLICT_STRATEGIES = [
  ...
  { value: "manual", label: "Manual Review", description: "Prompt for resolution on conflicts" },
] as const;
...
export default function SyncSettingsPage() {
  ...
  const loadSettings = useCallback(async () => {
    if (!token) return;
    try {
      const [settingsJson, syncJson] = await Promise.all([
        apiClient.get<{ data: Partial<SyncSettings> }>(`/settings/sync?workspace_id=${workspaceId}`),
        apiClient.get<{ meta?: { total?: number }; data?: Array<{ created_at: string }> }>(`/sync/?workspace_id=${workspaceId}&pending=true`),
      ]);

      if (settingsJson.data) {
        setSettings({ ...DEFAULT_SETTINGS, ...settingsJson.data });
      }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const json = await apiClient.get<{ meta?: { total?: number } }>(`/sync/?workspace_id=${workspaceId}&pending=true`);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await apiClient.put("/settings/sync", { workspace_id: workspaceId, ...settings });
      setSaved(true);
  ...
  return (
    <div className="mx-auto max-w-2xl p-6 space-y-8">
      ...
      <div className="flex items-center gap-2 text-sm text-zinc-500">
        <Link href={`/workspace/${workspaceId}/settings`} className="hover:text-white">
      ...
      <div>
        <h1 className="text-2xl font-bold text-white">Sync Configuration</h1>
        <p className="mt-1 text-sm text-zinc-500">Configure how this workspace syncs across devices</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-4">
          ...
          <p className={cn("mt-2 text-sm font-semibold", pendingOps > 0 ? "text-amber-400" : "text-white")}>
            {pendingOps} ops
          </p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-6 space-y-6">
        ...
        {/* Sync Interval */}
        <div>
          <label htmlFor="sync-interval" className="block text-sm font-medium text-zinc-300 mb-1.5">Sync Interval</label>
        ...
        {/* Conflict Strategy */}
        <div>
          <label htmlFor="conflict-resolution" className="block text-sm font-medium text-zinc-300 mb-1.5">Conflict Resolution</label>
          <div className="space-y-2">
            {CONFLICT_STRATEGIES.map((strategy) => (
              <button
                key={strategy.value}
                onClick={() => update("conflict_strategy", strategy.value)}
                className={cn(
        ...
        <Switch
          ...
          checked={settings.sync_on_connect}
          onCheckedChange={(checked) => update("sync_on_connect", checked)}
        />

        {/* Max Pending Ops */}
        <Slider
          label={`Max Pending Operations: ${settings.max_pending_ops}`}
          min={50}
          max={2000}
          step={50}
