  ...
  return (
    <div className="space-y-3 py-6">
      {Array.from({ length: 4 }).map((_, i) => (
        <div
          key={i}
          className="rounded-lg border-border bg-card p-4 space-y-2"
        >
    ...
    <div className="space-y-2">
      ...
      {results.map((result) => (
        <button
          ...
          className="group block w-full rounded-lg border-border bg-card p-4 text-left transition-colors card-hover"
        >
          <div className="flex items-start justify-between gap-3">
            ...
            <ArrowRight className="mt-1 h-4 w-4 shrink-0 text-muted group-hover:text-foreground" />
          </div>
        </button>
      ))}
    </div>
